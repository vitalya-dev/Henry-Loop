...
switch (state) {
  ...
  case "OPEN":
    ...
    if (gcounter_hover && gcounter_window && mouse_check_button_pressed(mb_left)) {
      scr_camera_set_size(gcounter_window.sprite_width + 10, gcounter_window.sprite_height + 10);
      scr_camera_follow(gcounter_window);
    }
    if (oscilloscope_hover && oscilloscope_window && mouse_check_button_pressed(mb_left)) {
      scr_camera_set_size(oscilloscope_window.sprite_width + 10, oscilloscope_window.sprite_height + 10);
      scr_camera_follow(oscilloscope_window);
    }
